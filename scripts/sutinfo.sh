...
distro="$(lsb_release -ds 2>/dev/null|| awk -F= '$1=="PRETTY_NAME" { print $2 ;}' /etc/*release || uname -om)"
...
if [[ "" != "$pf" ]] && [[ -f  /sys/class/net/$pf/address ]] ; then
	...
	if [ "$pf_mod" = "ice" ]; then
		ddp_version=$(dmesg | grep "ice $bus: \(The DDP package was successfully loaded\|DDP package already present on device\):" | tail -1 | sed "s/.*: //")
	fi
fi
...
if [ -n "$ddp_version" ]; then
	echo "  DDP version: $ddp_version"
fi
