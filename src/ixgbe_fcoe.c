...
int ixgbe_fcoe_ddp_put(struct net_device *netdev, u16 xid)
{
	...
	if (ddp->sgl)
		dma_unmap_sg(ixgbe_pf_to_dev(adapter), ddp->sgl, ddp->sgc,
			     DMA_FROM_DEVICE);
	if (ddp->pool) {
				...
				int target_mode)
{
	...
	dmacount = dma_map_sg(ixgbe_pf_to_dev(adapter), sgl, sgc, DMA_FROM_DEVICE);
	if (dmacount == 0) {
...
out_noddp_unmap:
	dma_unmap_sg(ixgbe_pf_to_dev(adapter), sgl, sgc, DMA_FROM_DEVICE);
out_noddp:
	put_cpu();
	return 0;
}
		   ...
		   struct sk_buff *skb)
{
	...
	case cpu_to_le32(IXGBE_RXDADV_STAT_FCSTAT_FCPRSP):
		dma_unmap_sg(ixgbe_pf_to_dev(adapter), ddp->sgl,
...
void ixgbe_free_fcoe_ddp_resources(struct ixgbe_adapter *adapter)
{
	...
	dma_unmap_single(ixgbe_pf_to_dev(adapter),
